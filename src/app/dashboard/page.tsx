    ...
    React.useEffect(() => {
        ...
        async function loadAgent() {
            try {
                ...
                const isPublished: boolean = data.is_published ?? false;

                // Show agent card ONLY if actually published
                if (uiConfig && isPublished) {
                    const modelCfg = AI_MODELS[DEFAULT_MODEL_ID];
                    setPublishedAgent({
                        id: session.user.id,
                        name: uiConfig.agentName ?? "My AI Agent",
                        primaryColor: uiConfig.primaryColor ?? "#7c3aed",
    ...
    return (
        <div className="flex min-h-screen bg-zinc-50 dark:bg-zinc-950">
            ...
            <main className={mainClass}>
                ...
                {/* Dashboard Content — full-width agent cards */}
                <div className="overflow-y-auto p-6">
                    ...
                    <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
                        {/* Loading state */}
                        {agentsLoading && (
                            <div className="col-span-3 flex h-48 items-center justify-center rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
                                ...
                                <div className="relative p-4">
                                    ...
                                    <div className="absolute right-3 top-3">
                                        {publishedAgent.is_published ? (
                                            ...
                                            <div className="flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-medium bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-300">
                                    ...
                                    <p className="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">
                                        {publishedAgent.is_published ? "Your published AI support agent" : "Draft — click to configure & publish"}
                                    </p>

                                    {/* Model Badge */}
                                    <div className="mt-2 flex items-center gap-1.5">
                                        <div className="inline-flex items-center gap-1 rounded-lg bg-zinc-100 px-2 py-0.5 text-[10px] font-semibold text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
                                            <Cpu className="h-2.5 w-2.5" />
                                            {publishedAgent.modelName}
                                        </div>
                                        <div className="inline-flex items-center gap-1 rounded-lg bg-zinc-100 px-2 py-0.5 text-[10px] font-medium text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">
                                            {publishedAgent.modelProvider}
                                        </div>
                                    </div>
                                    ...
                                    {publishedAgent.is_published ? (
                                        ...
                                        <div className="mt-2 inline-flex items-center gap-1 rounded-lg bg-amber-100 px-2 py-0.5 text-[10px] font-medium text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">
                                            <Sparkles className="h-2.5 w-2.5" />
                                            Not yet published
                                        </div>
                                    )}
                        ...
                        {!agentsLoading && !publishedAgent && (
                            <div className="col-span-3 flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-zinc-200 bg-white/50 py-12 text-center dark:border-zinc-800 dark:bg-zinc-900/50">
